...
public class DemoTransformer {
	...
	public static void test3(){
		Transformer<Employee,Level> trans = new Transformer<Employee,Level>(){

			public Level transform(Employee emp) {
				if (emp.getSalary()>=10000){
					return new Level(emp.getName(),"养身中");
				}else{
					return new Level(emp.getName(),"卖身中");
				}
		...
		Collection<Level>  result=CollectionUtils.collect(list, trans);
		for(Level time:result){
	 ...
	 * 解耦，业务处理与判断进行分类 
	 */
	public static void testSwitchTransformer(){
